...
public abstract class AbstractJsonCodec<O, A> implements Codec<O> {
    ...
    protected Object readValue(BsonReader reader, DecoderContext ctx) {
        ...
        return switch (type) {
            case NULL -> readNull(reader, ctx);
            case ARRAY -> readArray(reader, ctx);
            case BINARY -> readBinary(reader, ctx);
            case BOOLEAN -> readBoolean(reader, ctx);
            case DATE_TIME -> readDateTime(reader, ctx);
            case DB_POINTER -> readDbPointer(reader, ctx);
            case DOCUMENT -> readDocument(reader, ctx);
            case DOUBLE -> readDouble(reader, ctx);
            case INT32 -> readInt32(reader, ctx);
            case INT64 -> readInt64(reader, ctx);
            case MAX_KEY -> readMaxKey(reader, ctx);
            case MIN_KEY -> readMinKey(reader, ctx);
            case JAVASCRIPT -> readJavaScript(reader, ctx);
            case JAVASCRIPT_WITH_SCOPE -> readJavaScriptWithScope(reader, ctx);
            case OBJECT_ID -> readObjectId(reader, ctx);
            case REGULAR_EXPRESSION -> readRegularExpression(reader, ctx);
            case STRING -> readString(reader, ctx);
            case SYMBOL -> readSymbol(reader, ctx);
            case TIMESTAMP -> readTimeStamp(reader, ctx);
            case UNDEFINED -> readUndefined(reader, ctx);
            case DECIMAL128 -> readNumberDecimal(reader, ctx);
            default -> throw new IllegalStateException("Unknown bson type " + type);
        };
    }
